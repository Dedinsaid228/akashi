...
void AOClient::cmdFlip(int argc, QStringList argv)
{
    ...
    sendServerMessageArea("[" + QString::number(clientId()) + "] " + getSenderName(clientId()) + " flipped a coin and got " + l_face + ".");
}
...
void AOClient::cmdTimer(int argc, QStringList argv)
{
    ...
    std::shared_ptr<AOPacket> l_show_timer = PacketFactory::createPacket("TI", {QString::number(l_timer_id), "2"});
    std::shared_ptr<AOPacket> l_hide_timer = PacketFactory::createPacket("TI", {QString::number(l_timer_id), "3"});
    ...
    if (l_requested_time.isValid()) {
        ...
        std::shared_ptr<AOPacket> l_update_timer = PacketFactory::createPacket("TI", {QString::number(l_timer_id), "0", QString::number(QTime(0, 0).msecsTo(l_requested_time))});
    ...
    // Otherwise, update the state of the timer
        ...
        if (argv[1] == "start") {
            ...
            std::shared_ptr<AOPacket> l_update_timer = PacketFactory::createPacket("TI", {QString::number(l_timer_id), "0", QString::number(QTime(0, 0).msecsTo(QTime(0, 0).addMSecs(l_requested_timer->remainingTime())))});
            ...
            std::shared_ptr<AOPacket> l_update_timer = PacketFactory::createPacket("TI", {QString::number(l_timer_id), "1", QString::number(QTime(0, 0).msecsTo(QTime(0, 0).addMSecs(l_requested_timer->interval())))});
            l_is_global ? server->broadcast(l_update_timer) : server->broadcast(l_update_timer, areaId());
        }
...
void AOClient::cmdNoteCard(int argc, QStringList argv)
{
    ...
    sendServerMessageArea("[" + QString::number(clientId()) + "] " + getSenderName(clientId()) + " wrote a note card.");
}
...
void AOClient::cmdNoteCardClear(int argc, QStringList argv)
{
    ...
    if (!l_area->addNotecard(character(), QString()))
        sendServerMessageArea("[" + QString::number(clientId()) + "] " + getSenderName(clientId()) + " erased their note card.");
}
...
void AOClient::cmd8Ball(int argc, QStringList argv)
{
    ...
    if (ConfigManager::magic8BallAnswers().isEmpty()) {
        ...
        sendServerMessageArea("[" + QString::number(clientId()) + "] " + getSenderName(clientId()) + " asked the magic 8-ball, \"" + l_sender_message + "\" and the answer is: " + l_response);
    }
}
...
void AOClient::cmdVote(int argc, QStringList argv)
{
    ...
    if (argc == 0 && checkPermission(ACLRole::CM)) {
        ...
        if (l_area->isVoteStarted()) {
            ...
            for (AOClient *l_client : l_clients) {
                if ((l_area->lockStatus() == AreaData::LockStatus::FREE || l_area->invited().contains(l_client->clientId())) && l_client->m_ipid != m_ipid && l_client->areaId() == areaId()) {
                    l_client->m_can_vote = true;
                    l_client->m_vote_candidate = true;
                    l_client->m_vote_points = 0;
                    l_candidates_count++;
                    l_candidates += "\n[" + QString::number(l_client->clientId()) + "] " + getSenderName(l_client->clientId());
                }
            }
        ...
        sendServerMessageArea("[" + QString::number(clientId()) + "] " + getSenderName(clientId()) + " has voted.");
        l_target_client->m_vote_points++;
        m_can_vote = false;
        bool l_all_voted = true;
        const QVector<AOClient *> l_clients = server->getClients();
        for (AOClient *l_client : l_clients)
            if (l_client->areaId() == areaId() && l_client->m_can_vote)
                l_all_voted = false;
