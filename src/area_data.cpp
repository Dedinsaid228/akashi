...
void AreaData::addOwner(int f_id)
{
    m_owners.append(f_id);
    m_invited.append(f_id);
}

bool AreaData::removeOwner(int f_id)
{
    const QList<int> lastowners = m_owners;

    m_owners.removeAll(f_id);
    m_invited.removeAll(f_id);
...
bool AreaData::invite(int f_id)
{
    if (m_invited.contains(f_id))
        return false;
...
bool AreaData::uninvite(int f_id)
{
    if (!m_invited.contains(f_id))
        return false;
...
void AreaData::swapEvidence(int f_eviId1, int f_eviId2) { m_evidence.swapItemsAt(f_eviId1, f_eviId2); }
...
void AreaData::changeAmbience(const QString &f_newSong_r) { m_currentAmbience = f_newSong_r; }

QString AreaData::currentMusic() const { return m_currentMusic; }

QString AreaData::currentAmbience() const { return m_currentAmbience; }
