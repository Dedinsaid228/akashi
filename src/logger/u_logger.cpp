                       ...
                       const QString &f_hwid)
{
    ...
    QString l_logEntry = QString(m_logtext.value("login") + "\n")
                             .arg(l_time, l_success, f_ipid, f_char_name, f_ooc_name, f_uid, f_hwid);
    updateAreaBuffer(f_area_name, l_logEntry);
}
...
void ULogger::logKick(const QString &f_moderator, const QString &f_target_ipid, const QString &f_uid,
                      const QString &f_hwid)
{
    QString l_time = QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss");
    QString l_logEntry = QString(m_logtext.value("kick") + "\n")
                             .arg(l_time, f_moderator, f_target_ipid, f_uid, f_hwid);
    updateAreaBuffer("SERVER", l_logEntry);
}

void ULogger::logBan(const QString &f_moderator, const QString &f_target_ipid, const QString &f_duration, const QString &f_uid,
                     const QString &f_hwid)
{
    QString l_time = QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss");
    QString l_logEntry = QString(m_logtext.value("ban") + "\n")
                             .arg(l_time, f_moderator, f_target_ipid, f_duration, f_uid, f_hwid);
    updateAreaBuffer("SERVER", l_logEntry);
}
