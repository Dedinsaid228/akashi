...
{
    const QString old_char = current_char;
    bool argument_ok;
    ...
    server->areas.value(current_area)->LogChangeChar(current_char, ipid, hwid, showname, ooc_name, QString::number(id), old_char);
}
...
void AOClient::pktIcChat(AreaData* area, int argc, QStringList argv, AOPacket packet)
{
    ...
    if (wuso) {
        sendServerMessage("You cannot speak while you are affected by WUSO Mod.");
        return;
    }
    ...
    area->log(current_char, ipid, hwid, validated_packet, showname, ooc_name, QString::number(id));
    ...
    area->updateLastICMessageOwner(ipid);
...
void AOClient::pktOocChat(AreaData* area, int argc, QStringList argv, AOPacket packet)
{
    ...
    bool slient = slient_mod;

    if (message.length() == 0)
        return;

    if (message.length() > ConfigManager::maxCharacters()) {
    ...
    if (chillmod == true && message.length() > ConfigManager::maxCharactersChillMod()) {
        sendServerMessage("Your message is too long!");
        return;
    }
    ...
    if (message.at(0) == '/') {
        ...
        area->logCmd(current_char, ipid, hwid, command, cmd_argv, showname, ooc_name, QString::number(id));
        return;
    }
    else {
        if (auth == true && slient == false) {
            AOPacket final_packet("CT", {"[M]" + ooc_name, message, "0"});
    ...
    area->log(current_char, ipid, hwid, final_packet, showname, ooc_name, QString::number(id));
}
...
void AOClient::pktWtCe(AreaData* area, int argc, QStringList argv, AOPacket packet)
{
    ...
    if (QDateTime::currentDateTime().toSecsSinceEpoch() - last_wtce_time <= 2) {
        sendServerMessage("You use WTCE buttons a lot!");
        return;
    }
...
void AOClient::pktWebSocketIp(AreaData* area, int argc, QStringList argv, AOPacket packet)
{
    ...
    if (remote_ip.isLoopback()) {
        ...
        QHostAddress l_remote_ip = remote_ip;
    ...
    webclientConnected(ipid, remote_ip.toString());
}

void AOClient::pktModCall(AreaData* area, int argc, QStringList argv, AOPacket packet)
{
    if (!usemodcall) {
        sendServerMessage("You cannot use Mod Call anymore. Be patient!");
        return;
    }

    QString name = showname;
    if (showname.isEmpty())
        name = current_char;

    for (AOClient* client : server->clients) {
        if (client->authenticated)
            client->sendPacket("ZZ", {"[MODCALL] " + name  + " ("+ ipid + ") in " + server->area_names[current_area] + ": " + packet.contents[0]});
    }

    area->log(current_char, ipid, hwid, packet, showname, ooc_name, QString::number(id));
...
void AOClient::pktAddEvidence(AreaData* area, int argc, QStringList argv, AOPacket packet)
{
    ...
    AreaData::Evidence evi;

    if (area->eviMod() == AreaData::EvidenceMod::HIDDEN_CM && !argv[1].startsWith("<owner="))
        evi = {argv[0], "<owner=hidden>\n" + argv[1], argv[2]};
    else
        evi = {argv[0], argv[1], argv[2]};
...
void AOClient::updateEvidenceListHidCmNoCm(AreaData *area)
{
    ...
    for (const AreaData::Evidence &evidence : area_evidence) {
        evidence_list.append(evidence_format.arg(evidence.name, "", evidence.image));
    }
...
AOPacket AOClient::validateIcPacket(AOPacket packet)
{
    ...
    if (chillmod == true && incoming_args[4].toString().size() > ConfigManager::maxCharactersChillMod()) {
        sendServerMessage("Your message is too long!");
        return invalid;
    }
    ...
    if (is_disemvoweled) {
        QString disemvoweled_message = incoming_msg.remove(QRegExp("[AEIOUaeiouАУОИЭЫЯЮЕЁауоиэыяюеё]"));
    ...
    if (pos != incoming_args[5].toString()) {
        if (incoming_args[5].toString() == "hidden") {
    ...
    if (incoming_args[10].toString().contains("4")) {
        ...
        if ((obj_mod != 0 && obj_mod != 1 && obj_mod != 2 && obj_mod != 3) || (chillmod == true && obj_mod != 0))
            return invalid;
