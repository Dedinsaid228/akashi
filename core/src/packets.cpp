...
void AOClient::pktHardwareId(AreaData* area, int argc, QStringList argv, AOPacket packet)
{
    ...
    sendPacket("ID", {QString::number(m_id), "kakashi", QCoreApplication::applicationVersion()});
}
...
void AOClient::pktOocChat(AreaData* area, int argc, QStringList argv, AOPacket packet)
{
    ...
    bool l_auth = m_authenticated;
    bool l_slient = m_sneak_mod;
    QString l_ooc_name;

   if (l_auth == true && l_slient == false)
       l_ooc_name = "[M]" + m_ooc_name;
   else
       l_ooc_name = m_ooc_name;
    ...
    if (l_message.at(0) == '/') {
        ...
        int cmd_argc = l_cmd_argv.length();

        handleCommand(l_command, cmd_argc, l_cmd_argv);
        return;
    }
    else {
...
void AOClient::pktChangeMusic(AreaData* area, int argc, QStringList argv, AOPacket packet)
{
    ...
    for (const QString &l_song : qAsConst(server->m_music_list)) {
        if (l_song == l_argument || l_song == "~stop.mp3") { // ~stop.mp3 is a dummy track used by 2.9+
            ...
            AOPacket music_change("MC", {l_final_song, argv[1], m_showname, "1", "0", l_effects});
...
void AOClient::pktWtCe(AreaData* area, int argc, QStringList argv, AOPacket packet)
{
    ...
    if (QDateTime::currentDateTime().toSecsSinceEpoch() - m_last_wtce_time <= 2) {
...
void AOClient::pktAddEvidence(AreaData* area, int argc, QStringList argv, AOPacket packet)
{
    ...
    if (area->eviMod() == AreaData::EvidenceMod::HIDDEN_CM && !argv[1].startsWith("<owner="))
        l_evi = {argv[0], "<owner=hidden>\n" + argv[1], argv[2]};
    else
        l_evi = {argv[0], argv[1], argv[2]};
...
#else
        QSet<bool> matches = client->casing_preferences.toSet().intersect(l_needs_set);
#endif
...
void AOClient::updateEvidenceListHidCmNoCm(AreaData *area)
{
    ...
    const QList<AreaData::Evidence> l_area_evidence = area->evidence();
    for (const AreaData::Evidence &evidence : l_area_evidence) {
        l_evidence_list.append(l_evidence_format.arg(evidence.name, "", evidence.image));
    }
...
AOPacket AOClient::validateIcPacket(AOPacket packet)
{
    ...
    QString incoming_msg = dezalgo(l_incoming_args[4].toString().trimmed());

    if (!area->lastICMessage().isEmpty()
            && incoming_msg == area->lastICMessage()[4]
            && m_ipid       == area->lastICMessageOwner()
            && incoming_msg != "")
        return l_invalid;
    ...
    m_last_message = incoming_msg;
    l_args.append(incoming_msg);
    ...
    if (l_incoming_args.length() > 15) {
        ...
        if ((m_version.release == 2) && (m_version.major == 6 || m_version.major == 7 || m_version.major == 8)) {
            QString x_offset = m_offset.split("&")[0];
            l_args.append(x_offset);

            QString other_x_offset = l_other_offset.split("&")[0];
            l_args.append(other_x_offset);
        }
        else {
    ...
    if (l_incoming_args.length() > 19) {
        ...
        l_args.append(QString::number(sfx_loop));

        // screenshake
        int screenshake = l_incoming_args[20].toInt();

        if (screenshake != 0 && screenshake != 1)
            return l_invalid;

        l_args.append(QString::number(screenshake));
