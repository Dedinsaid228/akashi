...
void PacketMA::handlePacket(AreaData *area, AOClient &client) const
{
    ...
    if (is_kick) {
        ...
        Q_EMIT client.logKick(moderator_name, target->m_ipid, reason, QString::number(client.clientId()), client.m_hwid);
        ...
        Q_EMIT client.logBan(moderator_name, target->m_ipid, timestamp, reason, QString::number(client.clientId()), client.m_hwid);
