    ...
    for (AOClient* client : qAsConst(server->m_clients)) {
        if (client->m_authenticated && !client->m_sneak_mod) {
            ...
            if (ConfigManager::authType() != DataTypes::AuthType::SIMPLE)
                l_entries << "Moderator: " + client->m_moderator_name;

            l_entries << "OOC name: " + client->m_ooc_name;
            l_entries << "ID: " + QString::number(client->m_id);
            l_entries << "Area: " + QString::number(client->m_current_area);
            l_entries << "Character: " + client->m_current_char;
            l_online_count++;
        }
    }
...
void AOClient::cmdMOTD(int argc, QStringList argv)
{
    if (argc == 0) {
        ...
        if (checkAuth(ACLFlags.value("MOTD"))) {
            ...
            emit logCMD((m_current_char + " " + m_showname),m_ipid, m_ooc_name,"CHANGEMOTD",l_MOTD,server->m_areas[m_current_area]->name(), QString::number(m_id), m_hwid);
        }
        else {
...
void AOClient::cmdMute(int argc, QStringList argv)
{
    ...
    emit logCMD((m_current_char + " " + m_showname),m_ipid, m_ooc_name,"MUTE","Muted UID: " + QString::number(l_target->m_id),server->m_areas[m_current_area]->name(), QString::number(m_id), m_hwid);
}
...
void AOClient::cmdUnMute(int argc, QStringList argv)
{
    ...
    emit logCMD((m_current_char + " " + m_showname),m_ipid, m_ooc_name,"UNMUTE","Unmuted UID: " + QString::number(l_target->m_id),server->m_areas[m_current_area]->name(), QString::number(m_id), m_hwid);
}
...
void AOClient::cmdOocMute(int argc, QStringList argv)
{
    ...
    emit logCMD((m_current_char + " " + m_showname),m_ipid, m_ooc_name,"OOCMUTE","OOC muted UID: " + QString::number(l_target->m_id),server->m_areas[m_current_area]->name(), QString::number(m_id), m_hwid);
}
...
void AOClient::cmdOocUnMute(int argc, QStringList argv)
{
    ...
    emit logCMD((m_current_char + " " + m_showname),m_ipid, m_ooc_name,"OOCUNMUTE","OOC unmuted UID: " + QString::number(l_target->m_id),server->m_areas[m_current_area]->name(), QString::number(m_id), m_hwid);
}
...
void AOClient::cmdBlockWtce(int argc, QStringList argv)
{
    ...
    emit logCMD((m_current_char + " " + m_showname),m_ipid, m_ooc_name,"BLOCKWTCE","Blocked UID: " + QString::number(l_target->m_id),server->m_areas[m_current_area]->name(), QString::number(m_id), m_hwid);
}
...
void AOClient::cmdUnBlockWtce(int argc, QStringList argv)
{
    ...
    emit logCMD((m_current_char + " " + m_showname),m_ipid, m_ooc_name,"UNBLOCKWTCE","Unblocked UID: " + QString::number(l_target->m_id),server->m_areas[m_current_area]->name(), QString::number(m_id), m_hwid);
}
...
void AOClient::cmdAllowBlankposting(int argc, QStringList argv)
{
    ...
    if (l_area->blankpostingAllowed() == false) {
        ...
        emit logCMD((m_current_char + " " + m_showname),m_ipid, m_ooc_name,"FIRBIDBLANKPOST","",server->m_areas[m_current_area]->name(), QString::number(m_id), m_hwid);
    }
    else {
        sendServerMessageArea("[" + QString::number(m_id) + "] " + l_sender_name + " has set blankposting in the area to allowed.");
        emit logCMD((m_current_char + " " + m_showname),m_ipid, m_ooc_name,"ALLOWBLANKPOST","",server->m_areas[m_current_area]->name(), QString::number(m_id), m_hwid);
    }
}
...
void AOClient::cmdBanInfo(int argc, QStringList argv)
{
    ...
    const QList<DBManager::BanInfo> l_bans = server->db_manager->getBanInfo(l_lookup_type, id);
...
void AOClient::cmdReload(int argc, QStringList argv)
{
    ...
    emit logCMD((m_current_char + " " + m_showname),m_ipid, m_ooc_name,"RELOAD","",server->m_areas[m_current_area]->name(), QString::number(m_id), m_hwid);
}
...
void AOClient::cmdForceImmediate(int argc, QStringList argv)
{
    ...
    emit logCMD((m_current_char + " " + m_showname),m_ipid, m_ooc_name,"FORCEIMMEDIATE",l_state,server->m_areas[m_current_area]->name(), QString::number(m_id), m_hwid);
}
...
void AOClient::cmdAllowIniswap(int argc, QStringList argv)
{
    ...
    emit logCMD((m_current_char + " " + m_showname),m_ipid, m_ooc_name,"ALLOWINISWAP",l_state,server->m_areas[m_current_area]->name(), QString::number(m_id), m_hwid);
}
...
void AOClient::cmdPermitSaving(int argc, QStringList argv)
{
    ...
    emit logCMD((m_current_char + " " + m_showname),m_ipid, m_ooc_name,"ALLOWTESTIMONYSAVING","Target UID: " + QString::number(l_client->m_id),server->m_areas[m_current_area]->name(), QString::number(m_id), m_hwid);
}
...
void AOClient::cmdKickUid(int argc, QStringList argv)
{
    ...
    emit logCMD((m_current_char + " " + m_showname),m_ipid, m_ooc_name,"KICKUID","Kicked UID: " + QString::number(l_target->m_id),server->m_areas[m_current_area]->name(), QString::number(m_id), m_hwid);
}
...
void AOClient::cmdUpdateBan(int argc, QStringList argv)
{
    ...
    emit logCMD((m_current_char + " " + m_showname),m_ipid, m_ooc_name,"UPDATEBAN","Update info: " + argv[1] + " " + argv[2],server->m_areas[m_current_area]->name(), QString::number(m_id), m_hwid);
}
...
void AOClient::cmdClearCM(int argc, QStringList argv)
{
    ...
    emit logCMD((m_current_char + " " + m_showname),m_ipid, m_ooc_name,"CLEARCM","",server->m_areas[m_current_area]->name(), QString::number(m_id), m_hwid);
}
...
void AOClient::cmdIpidInfo(int argc, QStringList argv)
{
    ...
    const QList<DBManager::idipinfo> l_ipidinfo = server->db_manager->getIpidInfo(l_ipid);
...
void AOClient::cmdTakeTakedChar(int argc, QStringList argv)
{
    ...
    emit logCMD((m_current_char + " " + m_showname),m_ipid, m_ooc_name,"TAKETAKEDCHAR",l_str_en,server->m_areas[m_current_area]->name(), QString::number(m_id), m_hwid);
}
...
void AOClient::cmdBlind(int argc, QStringList argv)
{
    ...
    emit logCMD((m_current_char + " " + m_showname),m_ipid, m_ooc_name,"BLIND","Blinded UID: " + QString::number(l_target->m_id),server->m_areas[m_current_area]->name(), QString::number(m_id), m_hwid);
}
...
void AOClient::cmdUnBlind(int argc, QStringList argv)
{
    ...
    emit logCMD((m_current_char + " " + m_showname),m_ipid, m_ooc_name,"UNBLIND","Unblinded UID: " + QString::number(l_target->m_id),server->m_areas[m_current_area]->name(), QString::number(m_id), m_hwid);
}
...
void AOClient::cmdSneakMod(int argc, QStringList argv)
{
    ...
    emit logCMD((m_current_char + " " + m_showname),m_ipid, m_ooc_name,"SNEAKMOD",l_str_en,server->m_areas[m_current_area]->name(), QString::number(m_id), m_hwid);
}
...
void AOClient::cmdToggleWebUsersSpectateOnly(int argc, QStringList argv)
{
    ...
    emit logCMD((m_current_char + " " + m_showname),m_ipid, m_ooc_name,"TOGGLEWUSO",l_str_en,server->m_areas[m_current_area]->name(), QString::number(m_id), m_hwid);
}
...
void AOClient::cmdRemoveWebUsersSpectateOnly(int argc, QStringList argv)
{
    ...
    emit logCMD((m_current_char + " " + m_showname),m_ipid, m_ooc_name,"REMOVEWUSO","Target UID: " + QString::number(l_target->m_id),server->m_areas[m_current_area]->name(), QString::number(m_id), m_hwid);
}
