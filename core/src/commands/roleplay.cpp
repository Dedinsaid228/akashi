...
void AOClient::cmdTimer(int argc, QStringList argv)
{
    ...
    // Otherwise, update the state of the timer
        ...
        if (argv[1] == "start") {
            ...
            AOPacket update_timer("TI", {QString::number(l_timer_id), "0", QString::number(QTime(0,0).msecsTo(QTime(0,0).addMSecs(l_requested_timer->remainingTime())))});
            l_is_global ? server->broadcast(l_show_timer) : server->broadcast(l_show_timer, m_current_area);
            l_is_global ? server->broadcast(update_timer) : server->broadcast(update_timer, m_current_area);
        }
            ...
            AOPacket update_timer("TI", {QString::number(l_timer_id), "1", QString::number(QTime(0,0).msecsTo(QTime(0,0).addMSecs(l_requested_timer->interval())))});
            l_is_global ? server->broadcast(update_timer) : server->broadcast(update_timer, m_current_area);
        }
