...
void AOClient::cmdFlip(int argc, QStringList argv)
{
    ...
    if (showname.isEmpty() && current_char.isEmpty())
        sendServerMessageArea("[" + QString::number(id) + "] " + "Spectator flipped a coin and got " + face + ".");
    else if (showname.isEmpty())
        sendServerMessageArea("[" + QString::number(id) + "] " + current_char + " flipped a coin and got " + face + ".");
    else
        sendServerMessageArea("[" + QString::number(id) + "] " + showname + " flipped a coin and got " + face + ".");
}
...
void AOClient::cmdNoteCard(int argc, QStringList argv)
{
    ...
    if (showname.isEmpty() && current_char.isEmpty())
        sendServerMessageArea("[" + QString::number(id) + "] " + "Spectator wrote a note card.");
    else if (showname.isEmpty())
        sendServerMessageArea("[" + QString::number(id) + "] " + current_char + " wrote a note card.");
    else
        sendServerMessageArea("[" + QString::number(id) + "] " + showname + " wrote a note card.");
}
...
void AOClient::cmdNoteCardClear(int argc, QStringList argv)
{
    ...
    if (!area->addNotecard(current_char, QString())) {
        if (showname.isEmpty() && current_char.isEmpty())
            sendServerMessageArea("[" + QString::number(id) + "] " + "Spectator erased their note card.");
        else if (showname.isEmpty())
            sendServerMessageArea("[" + QString::number(id) + "] " + current_char + " erased their note card.");
        else
            sendServerMessageArea("[" + QString::number(id) + "] " + showname + " erased their note card.");
    }
}
...
void AOClient::cmdNoteCardReveal(int argc, QStringList argv)
{
    ...
    area->logCmdAdvanced(current_char, ipid, hwid, "NOTECARD REVEAL", message, showname, ooc_name, QString::number(id));
}
...
void AOClient::cmd8Ball(int argc, QStringList argv)
{
    ...
    else if (showname.isEmpty() && current_char.isEmpty()) {
        QString response = ConfigManager::magic8BallAnswers()[(genRand(1, ConfigManager::magic8BallAnswers().size() - 1))];
        QString sender_name = "Spectator";
        QString sender_id = "[" + QString::number(id) + "] ";
        QString sender_message = argv.join(" ");

        sendServerMessageArea(sender_id + sender_name + " asked the magic 8-ball, \"" + sender_message + "\" and the answer is: " + response);
    }
    else if (showname.isEmpty()) {
        QString response = ConfigManager::magic8BallAnswers()[(genRand(1, ConfigManager::magic8BallAnswers().size() - 1))];
        QString sender_name = current_char;
        QString sender_id = "[" + QString::number(id) + "] ";
        QString sender_message = argv.join(" ");
